<?php
namespace App\Lib\Support\WxSDK;

use App\Lib\Support\WxSDK\Utils\HttpCurl;
use App\Lib\Support\WxSDK\Utils\Error;
use App\Lib\Support\WxSDK\Utils\SHA1;
use App\Lib\Support\WxSDK\Utils\Xml;
...
class Api 
{
    ...
    public function new_access_token()
    {
        ...
        if ($res === false) {
            \Trensy\Support\Log::error('Http Get AccessToken Error.');
            return false;            
        }

        // 异常处理: access_token获取失败
        if (!isset($res->access_token)) {
            \Trensy\Support\Log::error('Get AccessToken Error: ' . json_encode($res));
            return false;
        }
        $res->expires_in += time();
        return $res;
    ...
    public function get_access_token()
    {
        ...
        if ($this->get_access_token_diy !== false) {
            ...
            \Trensy\Support\Log::error('Not set get_tokenDiy method, AccessToken will be refreshed each time.');
        }        
        // 验证AccessToken是否有效        
        if (!$this->valid_access_token($token)) {
            ...
            if ($this->save_access_token_diy !== false) {
                ...
                \Trensy\Support\Log::error('Not set saveTokenDiy method, AccessToken will be refreshed each time.');
            }
        }
    ...
    public function get_jsapi_ticket()
    {
        ...
        if ($this->get_jsapi_ticket_diy !== false) {
            ...
            \Trensy\Support\Log::error('Not set getTicketDiy method, jsapi_ticket will be refreshed each time.');
        }
        ...
        if (!$this->valid_jsapi_ticket($ticket)) {
            ...
            if ($this->save_jsapi_ticket_diy !== false) {
                ...
                // 异常处理: 保存access_token方法未定义
                \Trensy\Support\Log::error('Not set saveTokenDiy method, jsapi_ticket will be refreshed each time.');
            }
        }
